/**
 * Response messages
 */

export const USER_DOES_NOT_EXIST = 'User does not exist';
export const INVALID_CREDENTIALS = 'Invalid credentials';
export const LOGIN_SUCCESS = 'Login success';
export const REGISTER_SUCCESS = 'Register success';
export const INVALID_TOKEN = 'Invalid token';
export const EMAIL_IS_ALREADY_EXISTS = 'Email is already exists';
export const NO_TOKEN = 'You should login';
export const JWT_EXPIRES = 'JWT expires';

export const NOT_FOUND = 'NOT_FOUND';
export const VALIDATION_FAILED = 'Validation failed';

export const CREATE_SUCCESS = 'Create success';
export const UPDATE_SUCCESS = 'Update success';

export const POST_SUCCESS = 'Post success';
export const LIKE_SUCCESS = 'Like success';
export const STAR_SUCCESS = 'Star success';

export const INVALID_TARGET = 'Invalid target';

/**
 * AUTH
 */

export const TOKEN_SECRET = 'myverysecrettext';
export const JWT_EXP = 50;
export const JWT_REFRESH_EXP = 10000;

/**
 * Server config
 */

export const MONGODB_URI = 'mongodb://localhost:27017/askdev';
export const PORT = 3000;